...
def ray_tracer(theta, src, ior, grad_ior, rmax, z_min, z_max, step, reflection_at_z = 0.0):
    """
    Adaptive Snell's law raytracer for plane-stratified media.
    """
    max_theta = 1e-4
    step_bound = min(step, np.abs(max_theta * ior(reflection_at_z) / grad_ior(reflection_at_z)))  # Lower bound for step size, to efficiently allocate memory. Assumes |dn/dz| takes on a maximum at z=0.
...
def get_rays(src, ior, grad_ior, rmax, z_min, z_max, mesh, step = 1.0):
    ...
    for theta in mesh:
        if 0 < theta < 180: # Ray-tracer requires horizontal propagation; filter out strictly vertical rays
            ...
            output = ray_tracer(theta, src, ior, grad_ior, rmax, z_min, z_max, step)
...
def get_special_bounds(src, ior, grad_ior, rmax, z_min, z_max, reflection_at_z, step = 1.0):
    """
    ...
    coords = np.concatenate(rays, axis = 1).T # Combine rays into one set of coordinates
